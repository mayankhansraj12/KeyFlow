...
|---|---|---|
| Source validation | Pass | `./Scripts/validate.sh`; 104 tests and the warnings-as-errors release build pass on the final working state |
...
| Platform integration tests | Pass locally / hardware gate remains | 104 tests plus AddressSanitizer and ThreadSanitizer cover injected permission, login item, audio, media, menu-bar preferences, runtime, screenshot events, thumbnail cache, compatibility policy, and switcher behavior; real TCC, Core Audio devices, ScreenCaptureKit, and exact AX activation remain clean-install/hardware gates |
...
- 2026-07-16: Rebuilt the application-only Shortcuts split view, made selection and recorder teardown deterministic, disabled text-selection cursors outside editable controls, and added a persisted five-choice template menu-bar icon picker using the KeyFlow touch motif by default. Schema 19 migration and native-symbol availability are covered by the 104-test suite; validation, AddressSanitizer, ThreadSanitizer, and the 27.64% overall/85.29% Core coverage gates pass.
- 2026-07-16: Narrowed menu-bar choices to five touch/pointer symbols with schema 20 migration for obsolete choices, made shortcut titles application-owned with no editable name field, and rebuilt Gestures and Window Switcher around aligned feature surfaces and a live-preview-first hierarchy. Packaged-app visual inspection and SwiftUI render checks pass at production and minimum window sizes; the suite now covers 107 cases with 51.97% overall and 90.36% Core line coverage.
